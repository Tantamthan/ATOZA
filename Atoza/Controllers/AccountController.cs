    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterDto dto)
        {
            ...
            if (isGoogleRegistration)
            {
                ...
                await _emailService.SendRegistrationSuccessAsync(
                    dto.Email,
                    dto.FullName,
                    dto.Role,
                    isPendingApproval: profile == null,
            ...
            await _emailService.SendRegistrationSuccessAsync(
                ...
                dto.FullName,
                dto.Role,
                isPendingApproval: string.Equals(dto.Role, "Teacher", StringComparison.OrdinalIgnoreCase),
                CancellationToken.None);
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordDto dto)
        {
            ...
            var resetToken = await _authService.CreatePasswordResetTokenAsync(dto.Email);
            if (resetToken != null)
            {
                var resetUrl = Url.Action(
                    nameof(ResetPassword),
                    "Account",
                    new { email = resetToken.Email, token = resetToken.Token },
                    Request.Scheme);

                if (!string.IsNullOrWhiteSpace(resetUrl))
                {
                    _ = Task.Run(() => _emailService.SendPasswordResetAsync(
                        resetToken.Email,
                        resetToken.FullName,
                        resetUrl,
                        CancellationToken.None));
            ...
            TempData["AuthMessage"] = "Neu email ton tai, chung toi da gui lien ket dat lai mat khau.";
        ...
        public IActionResult ResetPassword(string email, string token)
        {
            ...
            {
                TempData["AuthError"] = "Link dat lai mat khau khong hop le hoac da het han.";
                return RedirectToAction(nameof(Login));
            }
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordDto dto)
        {
            ...
            var (success, error, email, fullName) = await _authService.ResetPasswordAsync(dto);
            if (!success)
            {
                ViewBag.Error = error;
                return View(dto);
            ...
            if (!string.IsNullOrWhiteSpace(email))
            {
                ...
                _ = Task.Run(() => _emailService.SendPasswordChangedAsync(
                    email!,
            ...
            TempData["AuthMessage"] = "Mat khau da duoc dat lai. Vui long dang nhap bang mat khau moi.";
            return RedirectToAction(nameof(Login));
        }
